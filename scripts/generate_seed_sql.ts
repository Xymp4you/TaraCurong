...
const barangays = ['Poblacion', 'Carmen', 'Batomelong', 'Kalandagan', 'Bula', 'Tina', 'EJC Montilla', 'Calean', 'San Pablo', 'San Emmanuel', 'San Rafael', 'San Antonio', 'Grino', 'New Passi', 'New Isabela', 'Buenaflor', 'Lancheta', 'D'Ledesma', 'New Buenaflor', 'San Isidro', 'San Jose', 'Siguel', 'Tuka', 'Tinago', 'New Carmen', 'Lower Katungal'];
...
let sql = `-- Seed generated for TaraCurong
...
const tacurongEmployers = [
  'RD Corporation',
  'Citra Mina Seafood Corp.',
  'KCC Mall of Tacurong',
  'SM City Tacurong City',
  'Gaisano Mall of Tacurong',
  'Alliance Select Foods International',
  'Tacurong City Water District',
  'SOCOTECO II',
  'Mindanao State University - Tacurong',
  'Notre Dame of Tacurong College',
  'St. Elizabeth Hospital',
  'Tacurong City Doctors Hospital',
  'Sarangani Highlands Garden',
  'Greenleaf Hotel Tacurong',
...
for (let i=0; i<30; i++) {
  ...
  let name = tacurongEmployers[i % tacurongEmployers.length];
  let prov = 'Sultan Kudarat';
  let city = 'Tacurong City';
...
for(let i=0; i<118; i++) {
  ...
  let prefLoc1 = randChoice(barangays) + ', Tacurong City';
  let prefLoc2 = randChoice(barangays) + ', Tacurong City';
  let prefOcc1 = randChoice(jobTitles);
  let prefOcc2 = randChoice(jobTitles);
  
  sql += `INSERT INTO public.jobseekers (id, email, first_name, last_name, birth_date, gender, civil_status, is_pwd, phone, barangay, city, province, employment_status, is_active, job_seeking_status, profile_complete, profile_completeness, work_setup_preference, expected_salary_min, preferred_occupation_1, preferred_occupation_2, preferred_work_location_local_1, preferred_work_location_local_2, preference_full_time, preference_part_time) VALUES (${escapeSql(id)}, ${escapeSql(email)}, ${escapeSql(fname)}, ${escapeSql(lname)}, ${escapeSql('1990-01-01')}, ${escapeSql(gender)}, 'Single', false, ${escapeSql('09'+randInt(100000000, 999999999))}, ${escapeSql(brgy)}, 'Tacurong City', 'Sultan Kudarat', 'Unemployed', true, ${escapeSql(randChoice(jobseekingStatuses))}, true, 100, ${escapeSql(workSetupPref)}, ${randInt(15000, 25000)}, ${escapeSql(prefOcc1)}, ${escapeSql(prefOcc2)}, ${escapeSql(prefLoc1)}, ${escapeSql(prefLoc2)}, true, false);\n`;
  ...
  for(let j=0; j<numTrain; j++) {
    let tr_id = uuid();
    sql += `INSERT INTO public.jobseeker_trainings (id, jobseeker_id, course, hours_of_training, training_institution) VALUES (${escapeSql(tr_id)}, ${escapeSql(id)}, ${escapeSql(randChoice(['TESDA NC II', 'Leadership Training', 'First Aid', 'Customer Service Excellence', 'Web Development']))}, ${randInt(8, 120)}, 'TESDA Tacurong');\n`;
  }
}

const outputPath = path.join(__dirname, '..', 'data', 'seed_generated.sql');

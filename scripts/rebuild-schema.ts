...
async function rebuildSchema() {
  ...
  console.log('Connected to database. Rebuilding schema...');

  try {
    await sql.begin(async (sql) => {
      await sql`DROP TABLE IF EXISTS referrals CASCADE`;
      await sql`DROP TABLE IF EXISTS applications CASCADE`;
      await sql`DROP TABLE IF EXISTS jobs CASCADE`;
      await sql`DROP TABLE IF EXISTS jobseeker_experience CASCADE`;
      await sql`DROP TABLE IF EXISTS jobseeker_licenses CASCADE`;
      await sql`DROP TABLE IF EXISTS jobseeker_trainings CASCADE`;
      await sql`DROP TABLE IF EXISTS jobseeker_education CASCADE`;
      await sql`DROP TABLE IF EXISTS jobseeker_languages CASCADE`;
      await sql`DROP TABLE IF EXISTS jobseekers CASCADE`;
      await sql`DROP TABLE IF EXISTS users CASCADE`;
      await sql`DROP TABLE IF EXISTS employers CASCADE`;

      await sql`CREATE TABLE jobseekers (
        ...
        unemployed_due_to_calamity BOOLEAN DEFAULT false,
        ...
        preferred_work_location_local_1 TEXT, 
        preferred_work_location_local_2 TEXT, 
        preferred_work_location_local_3 TEXT, 
        preferred_work_location_overseas_1 TEXT, 
        preferred_work_location_overseas_2 TEXT, 
        preferred_work_location_overseas_3 TEXT, 
        
        other_skills JSONB, 
        other_skills_others TEXT,
      ...
      await sql`CREATE TABLE jobseeker_languages (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      await sql`CREATE TABLE jobseeker_education (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        jobseeker_id UUID REFERENCES jobseekers(id) ON DELETE CASCADE,
      ...
      await sql`CREATE TABLE jobseeker_trainings (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        jobseeker_id UUID REFERENCES jobseekers(id) ON DELETE CASCADE,
        course TEXT,
      ...
      await sql`CREATE TABLE jobseeker_licenses (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        jobseeker_id UUID REFERENCES jobseekers(id) ON DELETE CASCADE,
        eligibility TEXT, 
        date_taken DATE,
      ...
      await sql`CREATE TABLE jobseeker_experience (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        jobseeker_id UUID REFERENCES jobseekers(id) ON DELETE CASCADE,
      ...
      await sql`CREATE TABLE employers (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        email TEXT UNIQUE NOT NULL,
        password_hash TEXT NOT NULL,
      ...
      await sql`CREATE TABLE applications (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      await sql`CREATE TABLE referrals (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        job_id UUID REFERENCES jobs(id) ON DELETE CASCADE,
        jobseeker_id UUID REFERENCES jobseekers(id) ON DELETE CASCADE,
        employer_id UUID REFERENCES employers(id) ON DELETE CASCADE,

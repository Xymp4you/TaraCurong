...
export function JobCard({ job }: JobCardProps) {
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    const catchyMessage = `🚀 Hot Job Alert! Check out this opening for ${job.positionTitle} at ${job.employerName}. Find your next career move on TaraCurong! 💼✨\n\nApply here: ${url}`;
    
    try {
      if (navigator.share) {
        await navigator.share({
          title: `${job.positionTitle} | TaraCurong`,
          text: catchyMessage,
  ...
  return (
    ...
    >
      <Link href={`/jobseeker/jobs/${job.id}`}>
        <Card className="group relative overflow-hidden p-6 h-full flex flex-col bg-white/50 backdrop-blur-sm border-slate-200/60 hover:border-teal-500/50 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300">
          {/* Background Decorative Gradient */}
          <div className="absolute -right-12 -top-12 h-24 w-24 rounded-full bg-teal-500/5 blur-2xl transition-all group-hover:bg-teal-500/10" />
          
          <div className="flex-1">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div className="space-y-1 flex-1">
                <h3 className="font-bold text-lg text-slate-900 line-clamp-2 leading-tight group-hover:text-teal-700 transition-colors">
                  {job.positionTitle}
                </h3>
                <p className="text-sm font-medium text-slate-500">{job.employerName}</p>
              </div>
              <div className="flex flex-col items-end gap-1.5">
                <Badge variant="secondary" className="bg-teal-50 text-teal-700 border-teal-100 font-medium px-2.5 py-0.5 rounded-full text-[10px] uppercase tracking-wider">
                  {job.employmentType}
                </Badge>
                {job.workType && (
            ...
            <div className="grid grid-cols-1 gap-2.5 my-5">
              <div className="flex items-center gap-2.5 text-sm text-slate-600">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-50 text-slate-400 group-hover:bg-teal-50 group-hover:text-teal-600 transition-colors">
              ...
              <div className="flex items-center gap-2.5 text-sm text-slate-600">
                ...
                {job.startingSalary ? (
                  <span className="font-medium text-slate-700">{job.startingSalary}</span>
                ) : (
                  <span
                    className="font-medium text-amber-700"
                    title="The employer did not disclose a starting salary. Ask before accepting an offer."
                  >
          ...
          <div className="mt-6 flex gap-2">
            <Button 
              className="flex-1 h-10 bg-slate-900 hover:bg-teal-600 text-white font-semibold transition-all duration-300 shadow-sm hover:shadow-blue-200"
            >
            ...
            <Button 
              ...
              className={`flex-1 h-10 font-semibold transition-all duration-300 ${
                isSaved 
                  ...
                  : "border-slate-200 hover:border-teal-100 hover:bg-teal-50 hover:text-teal-700"
              }`}
            >
              {isSaved ? (
                <>
            ...
            <Button 
              ...
              size="icon"
              onClick={handleShare}
              className="h-10 w-10 shrink-0 border-slate-200 hover:border-teal-100 hover:bg-teal-50 hover:text-teal-700 transition-all duration-300"
            >
